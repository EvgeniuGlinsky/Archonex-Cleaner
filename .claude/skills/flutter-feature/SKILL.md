...
- The pure part of a feature gets its own folder and no I/O at all: `data/rules/` holds the platform tables, `ProtectedPaths` and `DeletionGuard`, and next door `OffLimitsPaths`, `OptimizeGuard` and `SavingsEstimator`. Splitting it that way is what makes the dangerous half unit-testable and the half that touches the disk a loop with nothing to decide.
- A parser that reads a format rather than a file goes in `data/probes/`, over an abstraction rather than over `dart:io`: `MediaProbeReader` takes a `ByteSource`, so the header parsers are pure, their fixtures are byte lists written out in the test file, and the one class that opens a real file — `FileByteSource` — sits in `data/file_system/` with the rest of the I/O. A checked-in binary fixture proves nothing, because the whole risk in a header parser is an offset being wrong in a way that still yields a plausible number.
- A collaborator that is a strategy rather than a repository goes in a folder named for what it does — `data/encoders/` — declares an `abstract interface class`, and takes a benign sibling for the platforms that cannot do it (`UnavailableEncoder`). It is not a repository and nothing above `data/` knows it exists; which one is used is settled on the platform boundary like everything else. It is also the one interface outside `domain/` that a test may fake, and the reason is in `CLAUDE.md`.
